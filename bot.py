...
OWNER_ID = int(os.environ.get("OWNER_ID", "0"))  # Numeric ID
...
async def check_links(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for dom in domains:
        if dom not in ALLOWED_DOMAINS:
            ...
            admin_username = context.bot.get_me().username or "Admin"
...
# ---------------- Owner Commands ----------------
async def resolve_user_id(context, update, target: str):
    """Fetch numeric user ID from username or ID"""
    if target.isdigit():
        return int(target)
...
async def ban_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        ...
        return
    target_id = await resolve_user_id(context, update, context.args[0])
    if not target_id:
        await update.message.reply_text("❌ User খুঁজে পাওয়া যায়নি।")
        return
    ban_user_db(target_id, reason="manual by owner")
    try: await context.bot.ban_chat_member(update.effective_chat.id, target_id)
    except: pass
    await update.message.reply_text(f"✅ User {target_id} কে ব্যান করা হলো।")
...
async def unban_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try: await context.bot.unban_chat_member(update.effective_chat.id, target_id)
    except: pass
    await update.message.reply_text(f"✅ User {target_id} কে আনব্যান করা হলো।")
...
async def kick_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        ban_user_db(target_id, reason="kick by owner")
        admin_username = update.effective_user.username or "Admin"
        await context.bot.send_message(chat_id=target_id,
            text=f"⚠️ আপনি '{update.effective_chat.title}' গ্রুপ থেকে @{admin_username} এর কারণে সরানো হয়েছে।"
        )
        await update.message.reply_text(f"✅ User {target_id} কে গ্রুপ থেকে সরানো হয়েছে।")
    except: pass
...
async def add_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_banned(target_id):
        await update.message.reply_text("❌ User banned আছে, আগে unban করতে হবে।")
        return
    try:
        await context.bot.unban_chat_member(update.effective_chat.id, target_id)
        await update.message.reply_text(f"✅ User {target_id} কে গ্রুপে add করা হয়েছে।")
    except: pass
...
async def set_owner_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(f"✅ নতুন Owner সেট করা হয়েছে: {OWNER_ID}")
    except:
...
def main():
    ...
    app.add_handler(CommandHandler("set_owner", set_owner_cmd))
